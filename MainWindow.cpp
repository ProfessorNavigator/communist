			    ...
			    int varform)
{
  ...
  for (size_t i = begin; i < end; i++)
      ...
      else
	{
      ...
      if (varform == 0)
	{
	  msg_grid_vect.push_back (std::make_tuple (fr, msg[i], nullptr));
	}
      if (varform == 1)
	{
      ...
      if (selectedc != nullptr)
	{
	  if (fordelkey != "")
	      ...
	      if (oper != nullptr)
		{
		  ...
		  oper->friendDelPulse = [disp]
		  {
		    disp->emit ();
		  };

		  Contdelwin->signal_close_request ().connect ( [this, disp]
		  {
		    delete disp;
  ...
  if (oper == nullptr)
    {
      ...
      op->profReceived = [this, key, ind, disp1mtx]
      (std::string keyt, int indt)
	{
	  ...
	  *ind = indt;
      ...
      op->msgSent = [this, keysm, indsm, disp2mtx]
      (std::string key, int ind)
	{
	  ...
	  *indsm = ind;
      ...
      op->messageReceived = [this, keyrm, rp, disp3mtx]
      (std::string key, std::filesystem::path p)
	{
	  ...
	  *rp = p;
      ...
      op->filerequest = [this, keyfr, tm, fs, fn, disp4mtx]
      (std::string key, uint64_t time, uint64_t filesize, std::string filename)
	{
	  ...
	  *fn = filename;
      ...
      op->fileRejected = [this, keyfrr, disp5mtx]
      (std::string key)
	{
	  disp5mtx->lock();
	  *keyfrr = key;
      ...
      op->filercvd = [this, disp6mtx, keyfiler, filenmr]
      (std::string key, std::string filename)
	{
	  ...
	  *filenmr = filename;
      ...
      op->filehasherr = [this, disp7mtx, keyfilenr, filenmnr]
      (std::string key, std::string filename)
	{
	  disp7mtx->lock();
	  *keyfilenr = key;
	  *filenmnr = filename;
      ...
      op->filesentsig = [this, disp8mtx, keyfiles, filenms]
      (std::string key, std::string filename)
	{
	  disp8mtx->lock();
	  *keyfiles = key;
	  *filenms = filename;
      ...
      op->filesenterror = [this, disp9mtx, keyfileerror, filenmerror]
      (std::string key, std::string filename)
	{
	  disp9mtx->lock();
	  *keyfileerror = key;
	  *filenmerror = filename;
      ...
      op->ipv6signal = std::function<void
      (std::string)> (
	  std::bind (&MainWindow::formIPv6vect, this, std::placeholders::_1));

      op->ipv4signal = std::function<void
      (std::string)> (
	  std::bind (&MainWindow::formIPv4vect, this, std::placeholders::_1));

      op->ipv6signalfinished = [this]
      {
	this->disp10.emit ();
      };

      op->ipv4signalfinished = [this]
      {
	this->disp11.emit ();
      };

      op->filepartrcvdsig = [this, disp12mtx, keyfileprg, fileprgp, fileprgsz]
      (std::string key, std::filesystem::path p, uint64_t sz)
	{
	  ...
	  *fileprgsz = sz;
      ...
      op->filepartsendsig = [this, disp13mtx, keyfileprgs, fileprgps,
			     fileprgszs]
      (std::string key, std::filesystem::path p, uint64_t sz)
	{
	  disp13mtx->lock();
	  ...
	  *fileprgszs = sz;
      ...
      op->smthrcvdsig = [this, disp14mtx, keychcon, tmchcon]
      (std::string key, uint64_t tm)
	{
	  ...
	  *tmchcon = tm;
      ...
      op->friendDeleted = [this, disp15mtx, keyfrrem]
      (std::string key)
	{
	  disp15mtx->lock();
	  *keyfrrem = key;
	...
	this->friendRemoved (keyfrrem, disp15mtx);
      ...
      op->canceled = [this, dispv, disp1mtx, disp2mtx, disp3mtx, disp4mtx,
		      disp5mtx, disp6mtx, disp7mtx, disp8mtx, disp9mtx,
		      disp12mtx, disp13mtx, disp14mtx, disp15mtx, key, ind,
		      keysm, indsm, keyrm, rp, keyfr, tm, fs, fn, keyfrr,
		      keyfiler, filenmr, keyfilenr, filenmnr, keyfiles, filenms,
		      keyfileerror, filenmerror, keyfileprg, fileprgp,
		      fileprgsz, keyfileprgs, fileprgps, fileprgszs, keychcon,
		      tmchcon, keyfrrem]
      {
  ...
  if (clck->get_current_button () == 3)
    {
      ...
      msg_grid_vectmtx.lock ();
      auto itmgrv = std::find_if (msg_grid_vect.begin (), msg_grid_vect.end (),
				  [fr]
				  (auto &el)
				    {
				      return std::get<0>(el) == fr;
				    });
      if (itmgrv != msg_grid_vect.end ())
	{
	  ...
	  if (Menu == nullptr)
		...
		this->Rightgrid->attach_next_to (*(this->replgrid),
						 *(this->msgovl),
						 Gtk::PositionType::BOTTOM, 2,
						 1);
	      });
	  ...
	  if (msg_grid_vect.size () == 0)
	    {
	      ...
	      msg_grid_vect.push_back (std::make_tuple (fr, ploc, nullptr));
	    }
	  else
	    {
	      Gtk::Widget *widg = std::get<0> (
      ...
      if (lab == nullptr)
	{
	  ...
	  if (oper != nullptr)
		...
		this->Contdelwin->close ();
	      ...
	      oper->friendBlockedSig = [disp2]
	      {
		disp2->emit ();
	      };

	      Contdelwin->signal_close_request ().connect ( [this, disp, disp2]
	      {
		delete disp;
		delete disp2;
		this->Contdelwin->hide ();
...
void
MainWindow::deleteSettingsWindow (int apply)
{
  if (Set_Win)
    {
      delete Set_Win;
