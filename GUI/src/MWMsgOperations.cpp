  ...
  if (utxt != "" || mw->attachedfile != nullptr)
      ...
      if (mw->msg_win_gr != nullptr)
	{
	  ...
	  clck->signal_pressed ().connect (
	      sigc::bind (sigc::mem_fun (*this, &MWMsgOperations::creatReply),
			  fr, message, clck, mw));
	  std::string index = "";
			      ...
			      std::mutex *disp3mtx)
{
  ...
  if (frit != mw->friendvect.end ())
      ...
      if (labnumr)
	{
      ...
      clck->set_button (3);
      clck->signal_pressed ().connect (
	  sigc::bind (sigc::mem_fun (*mw, &MainWindow::contMenu), clck, but));
			    ...
			    Gtk::Widget *widg)
{
  ...
  confirm->signal_clicked ().connect (
      sigc::bind (sigc::mem_fun (*this, &MWMsgOperations::removeMsgFunc),
