				  ...
				  std::string selnick)
{
  ...
  int index = -1;
  mw->contmtx.lock ();
  auto iter = std::find_if (mw->contacts.begin (), mw->contacts.end (),
			    [&filename]
			    (auto &el)
			      {
		  ...
		  (Gtk::PositionType pos)
		    {
		      ...
		      mwl->contmtx.lock();
		      LibCommunist Lc;
		      msgl = Lc.listMessages(filename, selkey, mwl->Username, mwl->Password);
		      mwl->contmtx.unlock();
		      ...
		      if (pos == Gtk::PositionType::TOP)
			{
			  if (msgl.size() > 20)
			      ...
			      for(size_t i = mwl->msg_grid_vect.size(); i < end; i++)
				{
			      ...
			      mwl->formMsgWinGrid (tmp, 0, tmp.size (), grid, selkey, selnick, index, 1);
			    }
			}
