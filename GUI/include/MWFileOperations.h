...
public:
  ...
  ~MWFileOperations ();
  void
  fileDownloadProg (std::string *keyg, std::filesystem::path *filepathg,
		    uint64_t *cursizeg, std::mutex *mtx);
  void
  fileSendProg (std::string *keyg, std::filesystem::path *filepathg,
		uint64_t *cursizeg, std::mutex *mtx);
  void
...
private:
  ...
  void
  acceptButtonSlot (Gtk::Window *window, MainWindow *mwl, std::string keyloc,
