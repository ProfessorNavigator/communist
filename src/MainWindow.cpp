...
MainWindow::MainWindow()
{
  ...
  css_provider->load_from_path(
      Glib::ustring(Userthemepath + "/" + Theme + "/mainWindow.css"));
...
MainWindow::mainWindowClose()
{
  if(oper != nullptr)
      ...
      else
	{
      ...
      bar->set_name("clBar");
      bar->get_style_context()->add_provider(css_provider,
...
MainWindow::mainWindow()
{
  ...
  for(size_t i = 0; i < friendvect.size(); i++)
	...
	{
	  std::string key = std::string(std::get<2>(friendvect[i])->get_text());
  ...
  std::filesystem::path backgrpath = std::filesystem::u8path(
      std::string(Userthemepath + "/" + Theme + "/background.jpeg"));
  if(std::filesystem::exists(backgrpath))
    {
      bckgrnd->set_draw_func([this]
  ...
  Glib::ustring buffer = "<interface>"
      "<menu id='menubar'>"
      "	<submenu>"
      "		<attribute name='label'>" + Glib::ustring(gettext("Contacts"))
      + "</attribute>"
	  "			<section>"
	  "				<item>"
	  "					<attribute name='label'>"
      + Glib::ustring(gettext("Edit sent request table")) + "</attribute>"
	  "					<attribute name='action'>prefgr.editReqList</attribute>"
	  "				</item>"
	  "				<item>"
	  "					<attribute name='label'>"
      + Glib::ustring(gettext("Edit contacts to send by relay"))
      + "</attribute>"
	  ...
	  "	</submenu>"
	  "	<submenu>"
	  "		<attribute name='label'>" + Glib::ustring(gettext("Instruments"))
      + "</attribute>"
	  "			<section>"
	  "				<item>"
	  "					<attribute name='label'>"
      + Glib::ustring(gettext("Edit profile")) + "</attribute>"
	  "					<attribute name='action'>prefgr.editpr</attribute>"
	  "				</item>"
	  "				<item>"
	  "					<attribute name='label'>" + Glib::ustring(gettext("Own key"))
      + "</attribute>"
	  "					<attribute name='action'>prefgr.ownkey</attribute>"
	  "				</item>"
	  "                         <item>"
	  "					<attribute name='label'>" + Glib::ustring(gettext("Settings"))
      + "</attribute>"
	  ...
	  "	</submenu>"
	  "	<submenu>"
	  "		<attribute name='label'>" + Glib::ustring(gettext("About"))
      + "</attribute>"
	  "			<section>"
	  "				<item>"
	  "					<attribute name='label'>"
      + Glib::ustring(gettext("About Communist")) + "</attribute>"
  ...
  addfrmtx.lock();
  Addfriends = Lc.readRequestList(filename, Username, Password);
  addfrmtx.unlock();
  ...
  for(size_t i = 0; i < contacts.size(); i++)
    {
      ...
      std::get<1>(ttup) = keyc;
      yesmtx.lock();
      std::string logp = Lc.getContactMsgLog(homepath, Username, Password, ind,
					     filename);
	  ...
	  if(frprofvect.size() > 0)
	    {
	      ...
	      buttontr->signal_unrealize().connect([Menu]
	      {
		delete Menu;
	      });
	      ...
	      if(picture.size() > 0)
		{
		  ...
		  avatarpath =
		      std::filesystem::temp_directory_path().u8string();
                  ...
                  avatarpath =
                    std::filesystem::temp_directory_path().parent_path().u8string();
#endif
		  avatarpath = avatarpath + "/" + Lc.randomFileName() + ".jpeg";
		  ...
		  Glib::RefPtr<Gdk::Pixbuf> imagec =
		      Gdk::Pixbuf::create_from_file(Glib::ustring(avatarpath));
		  int drarw, drarh;
	      ...
	      else
		{
	      ...
	      auto itbll = std::find(blockedc.begin(), blockedc.end(), searchk);
	    ...
	    {
	      std::cerr << "Error on opening friends' profile" << std::endl;
	    }
	}
  ...
  drarfb->set_draw_func(
	  ...
	  Glib::ustring(Userthemepath + "/" + Theme + "/file-icon.png")));
  ...
  entcont->signal_key_pressed().connect(
      sigc::mem_fun(*this, &MainWindow::sendMsgByKeyBoard), false);
  ...
  if(spchst == "1" && spch != nullptr)
    {
	  ...
	  sigc::bind(sigc::mem_fun(*this, &MainWindow::spellCheck), textb));
    }
  ...
  sendb->signal_clicked().connect(
      sigc::bind(sigc::mem_fun(*this, &MainWindow::sendMsg), msgfrm));
...
MainWindow::spellCheck(Glib::RefPtr<Gtk::TextBuffer> textb)
{
  ...
  if(txt.size() > 0)
	...
	{
	  std::string word = txt.substr(std::get<0>(tv[i]), std::get<1>(tv[i]));
	  AuxFunc af;
	  word = af.utf8to(word);
  ...
  if(username != "" && password != "")
    {
      ...
      af.homePath(&homepath);
      LibCommunist Lc;
      int result = Lc.readSeed(homepath, Username, Password, seed);
      if(result >= 0)
	{
	  mainWindow();
	}
...
MainWindow::saveProfile()
{
  ...
  if(notcompl == 0)
      ...
      if(Image == 1)
	{
      ...
      af.homePath(&filename);
      LibCommunist Lc;
      int result = Lc.createProfile(Username, Password, filename, profvect,
				    seed);
      if(result >= 0)
	{
...
{
  ContactOperations cop(this);
  cop.formMsgWinGrid(msg, begin, end, grid, key, nick, index, varform);
}
  ...
  if(oper == nullptr)
    {
      ...
      std::filesystem::path *filenmerror = new std::filesystem::path;
      ...
      (std::string key, std::filesystem::path p)
	{
	  ...
	  *rp = p;
      ...
      oper->filerequest_signal = [this, keyfr, tm, fs, fn, disp4mtx]
      (std::string key, uint64_t time, uint64_t filesize, std::string filename)
	{
	  ...
	  *fn = filename;
      ...
      oper->fileRejected_signal = [this, keyfrr, filefrr, disp5mtx]
      (std::string key, std::filesystem::path p)
	{
	  disp5mtx->lock();
	  *keyfrr = key;
      ...
      (std::string key, std::filesystem::path filepath)
	{
	  ...
	  *filenmr = filepath;
      ...
      oper->filehasherr_signal = [this, disp7mtx, keyfilenr, filenmnr]
      (std::string key, std::filesystem::path filepath)
	{
	  disp7mtx->lock();
	  *keyfilenr = key;
      ...
      (std::string key, std::filesystem::path filepath)
	{
	  ...
	  *filenms = filepath;
      ...
      oper->filesenterror_signal = [this, disp9mtx, keyfileerror, filenmerror]
      (std::string key, std::filesystem::path filepath)
	{
	  disp9mtx->lock();
	  *keyfileerror = key;
	  *filenmerror = filepath;
      ...
      oper->ipv6_signal = std::bind(&MainWindow::formIPv6vect, this,
				    std::placeholders::_1);

      oper->ipv4_signal = std::bind(&MainWindow::formIPv4vect, this,
				    std::placeholders::_1);
	...
	this->disp11.emit();
      ...
      oper->filepartrcvd_signal = [this, disp12mtx, keyfileprg, fileprgp,
				   fileprgsz]
      (std::string key, std::filesystem::path p, uint64_t sz)
	{
	  ...
	  *fileprgsz = sz;
      ...
      oper->filepartsend_signal = [this, disp13mtx, keyfileprgs, fileprgps,
				   fileprgszs]
      (std::string key, std::filesystem::path p, uint64_t sz)
	{
	  disp13mtx->lock();
	...
	this->disp17.emit();
      ...
      sigc::connection con;
      con = disp1.connect(
	  sigc::bind(sigc::mem_fun(*this, &MainWindow::addFriendSlot), key, ind,
		     disp1mtx));
      dispv.push_back(con);
	...
	mop.msgRcvdSlot(keyrm, rp, disp3mtx);
      ...
      dispv.push_back(con);
      con = disp4.connect(
	  sigc::bind(sigc::mem_fun(*this, &MainWindow::fileRequestSlot), keyfr,
		     tm, fs, fn, disp4mtx));
      dispv.push_back(con);
      con = disp5.connect(
	...
	if(this->Contdelwin)
      ...
      oper->net_op_canceled_signal = [this, dispv, disp1mtx, disp2mtx, disp3mtx,
				      ...
				      keyfrr, filefrr, keyfiler, filenmr,
				      keyfilenr, filenmnr, keyfiles, filenms,
				      keyfileerror, filenmerror, keyfileprg,
				      fileprgp, fileprgsz, keyfileprgs,
				      fileprgps, fileprgszs, keychcon, tmchcon,
				      keyfrrem]
      {
...
MainWindow::addFriendSlot(std::string *keyt, int *conind, std::mutex *disp1mtx)
{
  ...
  disp1mtx->unlock();
  addfrmtx.lock();
  Addfriends.erase(std::remove_if(Addfriends.begin(), Addfriends.end(), [&key]
  (auto &el)
    {
      return el == key;
    }),
  ...
  conavmtx.lock();
  conavatars.erase(std::remove_if(conavatars.begin(), conavatars.end(), [&key]
  (auto &el)
    {
  ...
  if(itcont != contacts.end())
    {
      ...
      LibCommunist Lc;
      std::vector<std::tuple<std::string, std::vector<char>>> frprofvect;
      frprofvect = Lc.readFriendProfile(homepath, key, Username, Password);
      if(frprofvect.size() > 0)
	{
	  Gtk::Button *buttontr = Gtk::make_managed<Gtk::Button>();
	  buttontr->get_style_context()->add_provider(
	  ...
	  clck->set_button(3);
	  clck->signal_pressed().connect(
	      sigc::bind(sigc::mem_fun(*this, &MainWindow::contMenu), buttontr,
			 Menu));
	  buttontr->add_controller(clck);
	  buttontr->signal_unrealize().connect([Menu]
	  {
	  ...
	  std::vector<char> picture;
	  auto itfrpr = std::find_if(frprofvect.begin(), frprofvect.end(), []
	  (auto &el)
	    {
	      return std::get<0>(el) == "Avatar";
	    });
	  ...
	  if(picture.size() > 0)
	    {
	      std::string avatarpath;
#ifdef __linux
	      avatarpath = std::filesystem::temp_directory_path().u8string();
#endif
#ifdef _WIN32
              avatarpath =
                std::filesystem::temp_directory_path().parent_path().u8string();
	  ...
	  Gtk::Label *nicklab = Gtk::make_managed<Gtk::Label>();
	  itfrpr = std::find_if(frprofvect.begin(), frprofvect.end(), []
	  (auto &el)
	    {
	      return std::get<0>(el) == "Nick";
	    });
	...
	{
	  std::cerr << "Error on openning friend's profile (on rcving)"
	      << std::endl;
	}
      proffopmtx.unlock();
    }
...
MainWindow::editProfile()
{
  ...
  if(profvect.size() > 0)
      ...
      if(itprv != profvect.end())
	{
	  std::vector<char> val = std::get<1>(*itprv);
	  std::string valstr;
	  std::copy(val.begin(), val.end(), std::back_inserter(valstr));
      ...
      else
	{
	  ...
	  sigc::bind(sigc::mem_fun(*this, &MainWindow::editProfileClose),
		     window),
	  false);
      window->show();
    }
...
MainWindow::saveProfileEP(Gtk::Window *windowb)
{
  ...
  if(ent->get_text() == "")
      ...
      if(Image > 0)
	{
      ...
      LibCommunist Lc;
      proffopmtx.lock();
      int result = Lc.editProfile(Username, Password, homepath, profvect);
      proffopmtx.unlock();
      if(result >= 0)
	{
	  contmtx.lock();
...
void
MainWindow::fileRejectedSlot(std::string *keyr, std::filesystem::path *filer,
			     std::mutex *disp5mtx)
{
  MWFileOperations fop(this);
...
void
MainWindow::fileRcvdStatus(std::string *key, std::filesystem::path *filepath,
...
void
MainWindow::contMenu(int numcl, double x, double y, Gtk::Button *contbutton,
...
MainWindow::aboutProg()
{
  ...
  abbuf = Glib::ustring(
      gettext("Communist is simple \"peer to peer\" messenger.\n"
	      "Author Yury Bobylev.\n\n"
