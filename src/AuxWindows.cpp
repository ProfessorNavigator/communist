...
{
  mw->frvectmtx.lock();
  auto itfr = std::find_if(mw->friendvect.begin(),
                           mw->friendvect.end(),
                           [button]
                           (auto & el)
  {
  ...
  if(itfr != mw->friendvect.end())
    {
      std::string key = std::string(std::get<2>
                                    (*itfr)->get_text());
      mw->contmtx.lock();
      auto itcont = std::find_if(mw->contacts.begin(),
                                 mw->contacts.end(),
                                 [&key]
                                 (auto & el)
      {
      ...
      if(itcont != mw->contacts.end())
        {
          ...
          frprofvect;
          frprofvect = Lc.readFriendProfile(homepath, friendkey,
                                            mw->Username,
                                            mw->Password);
          ...
          if(itprv != frprofvect.end())
            {
              ...
              std::string line;
              std::copy(val.begin(), val.end(), std::back_inserter(line));
              surnamelab->set_markup("<i>" + Glib::ustring(
                                       line) + "</i>");
            }
          ...
          grid->attach_next_to(*close, *drar,
  ...
  if(mw->selectedc != nullptr)
    {
      Glib::RefPtr<Gtk::FileChooserNative> fcd =
        Gtk::FileChooserNative::create(gettext("File selection"),
                                       *mw,
...
void
AuxWindows::attachFileFunc(int rid,
                           Glib::RefPtr<Gtk::FileChooserNative> fcd,
                           MainWindow *mwl)
{
  if(rid == Gtk::ResponseType::ACCEPT)
    {
      ...
      mwl->frvectmtx.lock();
      auto itfr = std::find_if(mwl->friendvect.begin(),
                               mwl->friendvect.end(), [mwl]
                               (auto & el)
      {
      ...
      if(itfr != mwl->friendvect.end())
        {
          ...
          if(mwl->Rightgrid != nullptr
              && mwl->attachfbutton != nullptr
              && mwl->attachedfile == nullptr)
            {
              ...
              mwl->attachedfile->set_ellipsize(
...
AuxWindows::editAddFriends()
{
  ...
  std::vector<std::tuple<std::string, std::vector<char>>>
  profvect;
  profvect = Lc.readProfile(homepath, mw->Username,
                            mw->Password);
  mw->addfrmtx.lock();
  ...
  Gtk::ScrolledWindow *scrll =
    Gtk::make_managed<Gtk::ScrolledWindow> ();
  scrll->set_policy(Gtk::PolicyType::NEVER,
  ...
  std::vector<Gtk::Label*> *labvect = new
    ...
    if(mwl->activereq != nullptr)
      {
        ...
        mwl->addfrmtx.lock();
        mwl->Addfriends.erase(std::remove(mwl->Addfriends.begin(),
                                          mwl->Addfriends.end(), key), mwl->Addfriends.end());
        if(mwl->oper != nullptr)
          {
        ...
        mwl->addfrmtx.unlock();
        labvect->erase(
          std::remove_if(labvect->begin(), labvect->end(), [&key]
                         (auto & el)
        {
...
void
AuxWindows::addFriendsSelected(int n_press, double x,
                               double y,
  ...
  if(clck->get_current_button() == 3)
    {
      ...
      Gtk::PopoverMenu *Menu =
        Gtk::make_managed<Gtk::PopoverMenu> ();
      Menu->set_parent(*lab);
...
AuxWindows::addFriends()
{
  ...
  accept->signal_clicked().connect(
    sigc::bind(sigc::mem_fun(*this,
                             &AuxWindows::addFriendsFunc), window,
               otherkey, mw));
...
void
AuxWindows::addFriendsFunc(Gtk::Window *window,
                           Gtk::Entry *entry,
                           MainWindow *mwl)
{
  ...
  if(key.size() == 64)
    {
      ...
      profvect;
      LibCommunist Lc;
      profvect = Lc.readProfile(homepath, mwl->Username,
                                mwl->Password);
      mwl->addfrmtx.lock();
      mwl->Addfriends = Lc.readRequestList(homepath,
                                           mwl->Username,
                                           mwl->Password);
      auto it = std::find(mwl->Addfriends.begin(),
                          mwl->Addfriends.end(),
                          key);
...
void
AuxWindows::resendWindow(std::string othkey,
                         std::filesystem::path msgpath)
{
  ...
  conf->signal_clicked().connect(
    sigc::bind(sigc::mem_fun(*this, &AuxWindows::confResend),
               window,
               othkey, msgpath, mwl));
...
void
AuxWindows::confResend(Gtk::Window *window,
                       std::string othkey,
                       std::filesystem::path msgpath, MainWindow *mwl)
{
  if(mwl->resendactive.size() > 0)
    {
      LibCommunist Lc;
      std::string filename = Lc.openMessage(msgpath, othkey,
                                            mwl->Username,
                                            mwl->Password);
          ...
          for(size_t k = 0; k < mwl->resendactive.size(); k++)
            {
              ...
              Gtk::CheckButton *chbtn = std::get<1>
                                        (mwl->resendactive[k]);
              if(chbtn->get_active())
                {
                  if(chstr == "0")
                      ...
                      if(!std::filesystem::exists(p))
                        {
                          ...
                          errl->set_text(
                            gettext(
                              "File you want to send does not exists"));
...
AuxWindows::editRelayList()
{
  ...
  rellist = Lc.readRelayContacts(homepath, mw->Username,
                                 mw->Password);
  mw->proffopmtx.unlock();

  std::vector<std::tuple<Gtk::Label*, Gtk::CheckButton*>>
      *listv =
  ...
  apply->signal_clicked().connect(
    sigc::bind(sigc::mem_fun(*this,
                             &AuxWindows::editRelayListFunc), mw,
               listv, window));
...
void
AuxWindows::infoMessage(Glib::ustring username,
