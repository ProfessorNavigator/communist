      ...
      if(mw->msg_win_gr != nullptr)
        {
          ...
          if(mw->msg_grid_vect.size() == 0)
          ...
          else
            {
              ...
              mw->msg_win_gr->query_child(*widg, column, row, width,
                                          height);
          ...
          mw->contmtx.lock();
          auto cit = std::find_if(mw->contacts.begin(),
                                  mw->contacts.end(),
                                  [&key]
                                  (auto & el)
          {
...
void
MWMsgOperations::msgRcvdSlot(std::string *key,
                             ...
                             std::mutex *disp3mtx)
{
  ...
  mw->chifcmtx.lock();
  auto itchfc = std::find_if(mw->chifc.begin(),
                             mw->chifc.end(), [&keyloc]
                             (auto & el)
  {
  ...
  if(frit != mw->friendvect.end())
    {
      ...
      if(mw->grnotific)
        {
          Glib::RefPtr<Gio::Notification> notific =
            Gio::Notification::create(
              gettext("New message"));
          Glib::ustring msgnot;
          msgnot = gettext("Message from ") + std::get<4>
                   (*frit)->get_text();
      ...
      clck->set_button(3);
      clck->signal_pressed().connect(
        sigc::bind(sigc::mem_fun(*mw, &MainWindow::contMenu), clck,
                   but));
      Glib::RefPtr<Gio::SimpleActionGroup> acgroup =
        Gio::SimpleActionGroup::create();
      acgroup->add_action(
        "info",
        sigc::bind(sigc::mem_fun(*mw, &MainWindow::friendDetails),
                   but));
      ...
      if(itchfc != mw->chifc.end())
        {
          ...
          onl->get_style_context()->add_provider(
            mw->css_provider, GTK_STYLE_PROVIDER_PRIORITY_USER);
          onl->set_text(onlt);
          ngr->attach_next_to(*onl, *namelab,
      ...
      if(mw->friendvect.size() > 0)
        {
          mw->fordelgrid->insert_next_to(*(std::get<0>
                                           (mw->friendvect[0])),
      ...
      if(mw->fordelkey != keyloc)
      ...
      else
        {
          ...
          LibCommunist Lc;
          std::string filename = Lc.openMessage(ploc, keyloc,
                                                mw->Username,
                                                mw->Password);
          std::filesystem::path filepath = std::filesystem::u8path(
                                             filename);
                          ...
                          if(n == std::string::npos)
                          ...
                          else
                            {
                              line.erase(
                                0,
                                n
                                + std::string("TMFile Attachment ").size());
                              std::string fnm = line.substr(0,
                                                            line.find(" "));
                              line.erase(
                                0,
          ...
          if(mw->msg_grid_vect.size() == 0)
            {
              ...
              mw->msg_grid_vect.push_back(std::make_tuple(fr, ploc,
                                          nullptr));
            }
          else
            {
...
void
MWMsgOperations::creatReply(int numcl, double x, double y,
                            Gtk::Frame *fr,
                            Gtk::Label *message,
                            Glib::RefPtr<Gtk::GestureClick> clck,
                            MainWindow *mwl)
          ...
          if(Menu == nullptr)
            {
              ...
              acgroup->add_action("reply", [mwl, message]
              {
                ...
                mwl->Rightgrid->attach_next_to(*(mwl->replgrid),
                                               *(mwl->msgovl),
...
void
MWMsgOperations::removeMsg(std::string othkey,
                           ...
                           Gtk::Widget *widg)
{
  ...
  confirm->signal_clicked().connect(
    sigc::bind(sigc::mem_fun(*this,
                             &MWMsgOperations::removeMsgFunc),
               window, mw, othkey, msgpath, widg));
...
void
MWMsgOperations::removeMsgFunc(Gtk::Window *window,
                               MainWindow *mwl,
                               std::string othkey,
                               std::filesystem::path msgpath,
                               Gtk::Widget *widg)
{
  ...
  mwl->sentstatus.erase(
    std::remove_if(
      mwl->sentstatus.begin(), mwl->sentstatus.end(), [othkey,
          msgpath]
      (auto & el)
  {
  ...
  mwl->msg_grid_vect.erase(
    std::remove_if(mwl->msg_grid_vect.begin(),
  ...
  mwl->msg_grid_vectmtx.unlock();
  int column, row, width, height;
  mwl->msg_win_gr->query_child(*widg, column, row, width,
                               height);
...
void
MWMsgOperations::msgSentSlot(std::string *key,
                             std::filesystem::path *msgind,
                             std::mutex *disp2mtx)
{
  ...
  mw->sentstatmtx.lock();
  auto it = std::find_if(
              mw->sentstatus.begin(), mw->sentstatus.end(), [&keyloc,
                  &ind]
              (auto & el)
  {
