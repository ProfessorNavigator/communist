                                   ...
                                   uint64_t *cursizeg, std::mutex *mtx)
{
  ...
  auto it = std::find_if(
              mw->fileprogrvect.begin(), mw->fileprogrvect.end(), [key,
                  filepath]
              (auto & el)
  {
      ...
      if(mw->fileprogrvect.size() > 0)
        {
          ...
          mw->dld_grid->attach_next_to(*lab, *sibl,
          ...
          sibl = frprgb;
          mw->dld_grid->attach_next_to(*cncl, *sibl,
...
void
MWFileOperations::fileSendCncl(MainWindow *mwl,
                               std::string key,
                               std::filesystem::path filepath)
{
  mwl->fileprogrvectmtx.lock();
  auto it = std::find_if(
              mwl->fileprogrvect.begin(), mwl->fileprogrvect.end(), [key,
                  filepath]
              (auto & el)
  {
    if(std::get<0>(el) == key && std::get<1>(el) == filepath)
                                 ...
                                 std::mutex *dispNmtx, int variant)
{
  ...
  mw->frvectmtx.lock();
  auto itfrv = std::find_if(mw->friendvect.begin(),
                            mw->friendvect.end(),
                            [&keyloc]
                            (auto & el)
  {
  ...
  if(nick != "")
    {
      ...
      window->signal_close_request().connect([window, remf,
                                                      filenm]
      {
        if(remf != nullptr)
          {
...
void
MWFileOperations::fileRequestSlot(std::string *key,
                                  uint64_t *tm,
                                  ...
                                  std::mutex *disp4mtx)
{
  ...
  mw->blockfreq.erase(
    std::remove_if(mw->blockfreq.begin(),
                   mw->blockfreq.end(), [&curtime]
                   (auto & el)
  {
          ...
          if(itfrv != mw->friendvect.end())
            {
              ...
              mw->blockfreq.push_back(std::make_tuple(keyloc, tml, 0,
                                                      def));
...
void
MWFileOperations::acceptButtonSlot(Gtk::Window *window,
                                   MainWindow *mwl,
                                   ...
                                   std::string fnmloc, uint64_t fs)
{
  ...
  fcd->signal_response().connect(
    sigc::bind(sigc::mem_fun(*this,
                             &MWFileOperations::openButtonSlot), fcd,
...
void
MWFileOperations::rejectButtonSlot(Gtk::Window *window,
                                   MainWindow *mwl,
                                   std::string keyloc, uint64_t tml)
{
...
void
MWFileOperations::warningAcceptButton(std::filesystem::path
                                      fp,
                                      MainWindow *mwl, std::string keyloc,
  ...
  if(it != mwl->blockfreq.end())
    {
      ...
      cncl->signal_clicked().connect(
        sigc::bind(sigc::mem_fun(*this,
                                 &MWFileOperations::cancelRcvFile),
                   mwl, keyloc, fp));
...
void
MWFileOperations::cancelRcvFile(MainWindow *mwl,
                                std::string keyloc,
                                std::filesystem::path fp)
{
  mwl->fileprogrvectmtx.lock();
  auto it = std::find_if(
              mwl->fileprogrvect.begin(),
              mwl->fileprogrvect.end(), [keyloc, fp]
              (auto & el)
  {
  ...
  no->get_style_context()->add_provider(mwl->css_provider,
                                        GTK_STYLE_PROVIDER_PRIORITY_USER);
  no->signal_clicked().connect(sigc::mem_fun(*window,
                               &Gtk::Window::close));
