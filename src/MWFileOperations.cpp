			       ...
			       uint64_t *cursizeg, std::mutex *mtx)
{
  ...
  if(it != mw->fileprogrvect.end())
  ...
  else
    {
      ...
      lab->set_text(
	  gettext("Sending ") + Glib::ustring(filepath.filename().u8string()));
	  ...
	  sigc::bind(sigc::mem_fun(*this, &MWFileOperations::fileSendCncl), mw,
		     key, filepath));
      ...
      else
	{
	  ...
	  mw->dld_win->set_title(gettext("Downloading/sending file progress"));
				   ...
				   std::mutex *disp5mtx)
{
  ...
  disp5mtx->unlock();
  mw->frvectmtx.lock();
  auto itfrv = std::find_if(mw->friendvect.begin(), mw->friendvect.end(), [&key]
  (auto &el)
    {
      ...
      if(it == mw->blockfreq.end())
	{
	  ...
	  if(itfrv != mw->friendvect.end())
	      ...
	      else
		{
	      ...
	      lab1->set_text(nick + gettext(" is trying to send you a file."));
		  ...
		  sigc::bind(
		      sigc::mem_fun(*this, &MWFileOperations::acceptButtonSlot),
		  ...
		  sigc::bind(
		      sigc::mem_fun(*this, &MWFileOperations::rejectButtonSlot),
...
void
MWFileOperations::warningAcceptButton(std::filesystem::path fp, MainWindow *mwl,
				      std::string keyloc, uint64_t tml,
				      uint64_t fs)
{
  std::filesystem::remove_all(fp);
  ...
  if(it != mwl->blockfreq.end())
    {
      ...
      lab->set_margin_end(5);
      lab->set_text(
	  gettext("Downloading ") + Glib::ustring(fp.filename().u8string()));
      Gtk::ProgressBar *frprgb = Gtk::make_managed<Gtk::ProgressBar>();
      frprgb->set_halign(Gtk::Align::CENTER);
      frprgb->set_valign(Gtk::Align::START);
      frprgb->set_margin(5);
      ...
      cncl->set_margin(5);
      cncl->set_name("cancelRepl");
      cncl->get_style_context()->add_provider(mwl->css_provider,
					      GTK_STYLE_PROVIDER_PRIORITY_USER);
      cncl->set_icon_name("window-close-symbolic");
      ...
      else
	{
	  ...
	  mwl->dld_win->set_title(gettext("Downloading/sending file progress"));
	  mwl->dld_grid = Gtk::make_managed<Gtk::Grid>();
	  mwl->dld_grid->set_halign(Gtk::Align::CENTER);
	  mwl->dld_win->set_child(*(mwl->dld_grid));
	  mwl->dld_grid->attach(*lab, 0, 0, 1, 1);
